...
void MatchHandler::computeHomographyMatch(Frame &frSrc, Frame &frDest, Mat &homography,
...
void MatchHandler::spatialMatch(Frame &fr1, Frame &fr2,
                                float linkDistance,
...
//    SonarDescritor::drawDescriptorColor(bgrImg1,&sd1,Scalar(0,255,0),Scalar(0,255,0));
    SonarDescritor::drawDescriptorColor(temp,&sd2,Scalar(0,255,0),Scalar(0,255,0));
    ...
    gm.matcheSIFTCutDebug(&sd1,&sd2,matchs,matchInfo);

    SonarDescritor::drawVertexMatch(bgrImg1,bgrImg2,
                                    sd1,sd2,
                                    matchInfo,
                                    Scalar(0,255,225),Scalar(0,255,255),
                                    2);
...
void MatchHandler::matchWithOpticalFlow(Frame &frSrc, Frame &frDest,
                                        Mat &img1, Mat &img2)
{
#ifdef MATCHHANDLER_TRACKING_DEBUG
...
void MatchHandler::consecutiveMatch(vector<Frame> &frames, unsigned srcFrID, unsigned destFrID)
{
    #ifdef MATCHHANDLER_TRACKING_DEBUG
...
bool MatchHandler::consecutiveMatch(vector<Frame> &frames, unsigned srcFrID, unsigned srcGID,
...
void MatchHandler::loadMatchs(vector<Frame> &frames, Frame &srcFr, Frame &destFr, Mat &img1, Mat &img2, float precision)
{
    #ifdef MATCHHANDLER_TRACKING_DEBUG
...
bool MatchHandler::findGTMatch(Point2f *src, Point2f *dst, float squaredPrecision)
{
    #ifdef MATCHHANDLER_TRACKING_DEBUG
        cout << "MatchHandler::findGTMatch use!" << endl;
    #endif
    ...
    {
        if(fabs(GTMatchSrc[i].x - src->x) <= squaredPrecision
           && fabs(GTMatchSrc[i].y - src->y) <= squaredPrecision)
        {
            *src = GTMatchSrc[i];
            *dst = GTMatchDest[i];
            return true;
...
bool MatchHandler::findHMatch(Point2f *src, Point2f *dst,  float squaredPrecision)
{
    #ifdef MATCHHANDLER_TRACKING_DEBUG
        cout << "MatchHandler::findHMatch use!" << endl;
    #endif
    ...
    {
        if(fabs(HMatchSrc[i].x - src->x) <= squaredPrecision
           && fabs(HMatchSrc[i].y - src->y) <= squaredPrecision)
        {
            *src = HMatchSrc[i];
            *dst = HMatchDest[i];
            return true;
