...
SonarConfig::SonarConfig(const char *fileName):
    name("Default"),
    // Sonar Config.
    ...
    sonar_stdDevMultiply(2.f), // Number of times we multiply stdDev to work
    sonar_graphLinkDistance(250.f), // Distance to link two vertex
    sonar_graphNeigborDistanceRelativeLink(30.f), // Distance to link two vertex
    sonar_graphCreatorMode(FIXED_DISTANCE),

    sonar_pixelThreshold(250), // Threshold for pixel search (start floodfill) on 8bits normalized image
    sonar_searchThreshold(190), // Threshold to floodfill search on 16bits image
