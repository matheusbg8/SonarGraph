...
void Drawing::drawVertexMatch(Mat &colorImg, const Size2i &drawingArea,
                              ...
                              const vector<MatchInfoExtended::PUUPFF> &matchInfo,
                              ...
                              int thickness)
{
    ...
    {
        const MatchInfoExtended::PUUPFF &m = matchInfo[i];

        float r = mr*(e1[m.first.first]->p/maxEdist1),
              ang = e1[m.first.first]->ang*M_PI/180.f;
        ...
        if(drawMatchInfo)
        {
            sprintf(tempStr,"mID %u ; fs%.2f ; ss%.2f",
                    i , m.second.first,m.second.second);
        ...
        r = mr*(e2[m.first.second]->p/maxEdist2),
        ang = e2[m.first.second]->ang*M_PI/180.f;
...
void Drawing::drawVertecesMatchs(Mat bgrImg1, Mat bgrImg2,
                              ...
                              int thickness)
{
    ...
    for(unsigned iMatch = 0 ; iMatch < matchInfo.size(); iMatch++)
    {
        ...
        vector<MatchInfoExtended::PUUPFF> &emi = mi.edgeMatchInfo;
