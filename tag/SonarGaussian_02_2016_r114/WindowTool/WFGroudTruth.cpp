...
void WFGroudTruth::acceptHomographyMatchs()
{
    ...
    currentFrame.removeMatchToFrame(wt->currentRighFrame);

    // Do all homography matchs
    for(unsigned i = 0; i < gSrcList.size();i++)
    {
        // Do i-est homography match
        currentFrame.newMatch(gSrcList[i], wt->currentRighFrame ,gDestList[i]);
    }
...
void WFGroudTruth::keyPress(char c)
{
    switch(c) // Preview frame
    {
        ...
        case 'o': // Compute optical flow matchs
            matchHandler.matchWithOpticalFlow(*frame(wt->currentLeftFrame), *frame(wt->currentRighFrame),
...
void WFGroudTruth::renderFrameTogether(Mat &screen, const Scalar_<float> &el, const Scalar_<float> &er)
{
//    // Draw texts
//    char tempStr[200];
